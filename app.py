...
def draw_tsp_plot(path, title, color="orange"):
    ...
    if path:
        ...
        fig.add_trace(go.Scatter(
            ...
            hoverinfo="skip" # 선에 대한 인터랙션 제외
        ))
    
    fig.update_layout(
        template="plotly_white",
        # [수정] 축 고정: 확대 및 드래그 이동 비활성화
        xaxis=dict(
            showgrid=False, range=[-5, 105], 
        ...
        height=900,
        showlegend=False,
        dragmode=False, # 클릭 외 드래그 동작(Box Select 등) 비활성화
...
# [중요] 공통 config: 모드바 숨김 및 스크롤 줌 방지
chart_config = {
    'displayModeBar': False, # 상단 도구 모음 제거
    'scrollZoom': False,     # 마우스 휠 확대 방지
    'staticPlot': False      # 클릭 이벤트는 받아야 하므로 False 유지
}
...
with col_main:
    ...
    # 2. Nearest Neighbor (애니메이션 루프 내에서도 config 유지)
    with tabs[1]:
